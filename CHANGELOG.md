...
- **ThreadSanitizer with the panel on.** A fourth sanitiser leg, "TSan with Qt": the seam
  tests and the panel's Qt tests with every access instrumented, under xvfb. Qt is not
  instrumented, so `tools/tsan.supp` names its own hand-offs - the blocking queued invoke
  that builds and deletes a window on the Qt thread, and what its libraries call into libc -
  and nothing else: the seam is a `std::mutex` the sanitiser sees whole, and a race there is
  the real thing the leg exists to catch. The panel test's one tight timing bound is widened
  under the sanitiser, the measured value still in the message.
